...
function DateGuess($scope) {
  ...
  $scope.jsParseDate = function(title) {
    ...
    if (matches) {
      var shablon2 = /(\d{4})/g;
      matches2 = title.match(shablon2); //найти год
      var shablon3 = /(янв)|(фев)|(мар)|(апр)|(мая)|(май)|(июн)|(июл)|(авг)|(сен)|(окт)|(ноя)|(дек)/g;
      matches3 = title.match(shablon3); //найти месяц
      var shablon4 = /(\d{1,2})/g;
      matches4 = matches[0].match(shablon4); //найти дату
    ...
    if (matches) {
      ...
      if (week != 0) {
        mydate = prevWeekDay(mydate, week);
        answer = matches[0];
      }
    }
  ...
  function prevWeekDay(date, day) { //поиск пред дня недели
    (day = (Math.abs(+day || 0) % 7) + date.getDay()) > 0 && (day -= 7);
    return day && date.setDate(date.getDate() + day), date;
  };
